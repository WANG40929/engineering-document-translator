# Version 1.1.1 verification report

- Automated offline tests: 23 passed, covering PDF/DOCX/XLSX/CSV, cache trimming, model migration, DPAPI, incomplete-response recovery, placeholder validation and cache self-healing, PDF origin preservation, font-size stability, recursive batch splitting, exact PDF failure progress, and queue failure behavior.
...
- PDF layout regression: translated text retains its original top-left anchor, header fields use adjacent whitespace, and standalone long text wraps without tiny fonts.
- Placeholder regression: malformed or hallucinated `UDT_0000` markers trigger selective repair, while old cache entries are restored or refreshed individually.
